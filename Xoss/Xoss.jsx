...
import {Upload, Toast, Button, Icon} from 'antd-mobile';
...
export default class Xoss extends Component {
  ...
  beforeUpload = () => {
    if (this.state.defaultFileList.size >= this.max) {
      Toast.offline(I18n('uploadOnlyAllow') + this.max + I18n('files'));
      return false;
    }
    return true;
  };
  ...
  onUploadSuccess = () => {
    Toast.success(I18n('uploadSuccess'));
  };
  onUploadError = (res) => {
    Toast.fail(I18n('uploadFail') + I18n(':') + (res.response.error || ''));
  };
